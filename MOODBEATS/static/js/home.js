...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showMoodModal(title, message, showCamera = false) {
        ...
        savePlaylistBtn.classList.add('d-none');
    ...
    /**
     * Plays YouTube video in modal and records play history
     * @param {string} url - YouTube URL
    ...
    function playYouTubeVideo(url, title, artist) {
        ...
        // Record in history if mood is set
        if (currentMood) {
    ...
    /**
     * Sends played song data to server for history recording
     */
    ...
    // Add click handlers to mood option buttons
    const moodOptions = document.querySelectorAll('.mood-option');
    moodOptions.forEach(option => {
        option.addEventListener('click', function() {
            currentMood = this.textContent.trim().toLowerCase();
            console.log('Selected mood:', currentMood);
            
            // Visual feedback
            moodOptions.forEach(opt => {
                if (opt.style) opt.style.transform = 'translateY(0)';
            });
            if (this.style) this.style.transform = 'translateY(-5px)';
        });
    });
    ...
    if (savePlaylistBtn) {
        savePlaylistBtn.addEventListener('click', function() {
            alert('Playlist saved to your favorites!');
            moodModal.hide();
        });
    }
